...
float offsets[NUM_MOTORS] = {0, 0, 0, 0}; // Loaded from EEPROM, sets offset of zero from encoder position
...
PIDConfig pidConfig = {1.0, 0.0, 0.0};  // default values
...
rcl_publisher_t feedback_pubs[NUM_MOTORS];
std_msgs__msg__Float32 feedback_msgs[NUM_MOTORS];
...
float readEncoderPWM(uint8_t pin_index) {
  ...
  // Only the data section encodes angle, which is 4095 out of 4119 bits
  float angle = (pw / per) * 360.0f * (4095.0f / 4119.0f);
  return angle;
}

...
void timer_callback(rcl_timer_t *, int64_t) {
  ...
  if (now - last_control_time >= control_interval) {
    ...
    for (int i = 0; i < NUM_MOTORS; i++) {
      ...
      joint_state_msg.position.data[i] = current_angles[i] * (M_PI / 180.0); // convert to radians
    }

    rcl_publish(&angle_pub, &angle_msg, NULL);
    // joint_state_msg.header.stamp = rmw_uros_epoch_nanos();  // Optional: time stamp
